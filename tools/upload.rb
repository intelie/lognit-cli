...
if login.empty? then print "login: "; login = $stdin.readline.chomp end
if token.empty? then print "token: "; token = $stdin.readline.chomp end

# The file we want to upload
require 'pathname'
file = Pathname.new(ARGV[0])
desired_name = ARGV[2]
class Pathname; def type; `file -b #{to_s}`.chomp; end; end
...
req.set_form_data({
  ...
  'content_type' => file.type, 'file_name' => desired_name,
})
...
die("Github doens't want us to upload the file.") unless res.class == Net::HTTPOK
...
res = http.post(uri.path, *build_multipart_content({
  'key' => info['path'], 'Filename' => desired_name, 'policy' => info['policy'],
